...
*/
        ...
        {
            LogService.Info(this.GetType().ToString(), "Native pay mode 2 url is producing...");
            ...
            LogService.Info("扫描支付", "调用统一下单接口的返回结果：" + result.ToXml());
            string url = result.GetValue("code_url").ToString();//获得统一下单接口返回的二维码链接

            LogService.Info(this.GetType().ToString(), "Get native pay mode 2 url : " + url);
            return url;
        }
    }
}
